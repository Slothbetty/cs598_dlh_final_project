...
join `physionet-data.mimiciii_clinical.diagnoses_icd` dicd on dp.HADM_ID = dicd.HADM_ID
order by dp.ROW_ID),
density_calculation as (select rirui.ROW_ID, count(rirui.ICD9_CODE)/737 as rolled_up_icd_density from report_id_rolled_up_icd rirui
group by rirui.ROW_ID)
...
join `physionet-data.mimiciii_clinical.diagnoses_icd` dicd on dp.HADM_ID = dicd.HADM_ID
order by dp.ROW_ID)
select rirui.ICD9_CODE, count(rirui.ROW_ID)/240567 as report_percentage from report_id_rolled_up_icd rirui
